...
```{r, message=FALSE, warning=FALSE, fig.width=7, fig.height=5, fig.cap="Figure 1.1.2. net1"}
...
```{r, message=FALSE, warning=FALSE, fig.width=7, fig.height=5, fig.cap="Figure 1.2.2. net2"}
...
`ts_alternatives_ministep()` returns a list of all possible networks after all possible tie-changes available to ego#2 given network `net2`. If you look closely you will see that `options[[2]]` equals the original network (i.e. ego#2 decided not to change any tie). 
