...
ts_sims(startvalues = c(2,-1,2), net=ts_net2, statistics=list(ts_degree, ts_recip),nsims=1, p2step=c(1,0,0), chain = TRUE )
```
...
```{r}
ts_sims(startvalues = c(2,-1,2), net=ts_net2, statistics=list(ts_degree, ts_recip),nsims=1, p2step=c(0,1,0), chain = TRUE )
```
...
```{r}
ts_sims(startvalues = c(2,-2,2,2), net=ts_net2, statistics=list(ts_degree, ts_recip, ts_outAct),nsims=1, p2step=c(0,0,1), chain = TRUE )
```
