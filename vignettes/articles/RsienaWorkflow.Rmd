...
Model: Degree, reciprocity, transitive triplets, 3cycle, smoke similarity, alcohol alter, alcohol ego, alcohol ego X alcohol alter 
...
Settup up the algorithm and define the model
```{r, eval=FALSE}
myalgorithm <- sienaAlgorithmCreate(cond=FALSE) #toggle set conditional to retrieve the rate parameter in theta!
...
# 5.1. Estimate via `RsienaTwoStep`
...
Define the model. Include all names of the statistics in a list. If the statistic requires a covariate use a list with the first element the name of the statistic and the second element the name of the covariate. This should be the same name as used in your dataset. 
...
We could estimate the parameters and the SE separately.This means we set `phase3` to `FALSE`. 


```{r, eval=FALSE}
ts_ans1a <- ts_estim(net1 = s501, net2= s502, statistics = STATS, ccovar = DF, p2step=c(1,0,0), parallel = TRUE, phase1 = TRUE, phase3 = FALSE)
```

We could also start with the data objects of RSiena. Note that we still perform subphase1 ourselves.
...
```{r, eval=FALSE}
ts_ans1c <- ts_estim(ans = ans3, phase3 = FALSE, nite=30, parallel = TRUE)
```
...
```{r, eval=TRUE, echo=FALSE}
load("./data/processed/20231009ts_ans1c.Rdata")
ts_ans1c <- x
rm(x)
```
...
If the lines keep oscillating around a specific value, try to increase the `b` parameter. Conversely, if the lines did not converge but you see no oscillation you could decrase the `b` parameter. 
...
Okay but let us move on to phase3. For reliable SE and tstats I think you really need to set `itef3` to 1000. 

Because we want to assess the GOF later, make sure to set `returnDeps = TRUE`
```{r}
#retrieve the estimates from phase2. 

ESTIM <- ts_ans1c[nrow(ts_ans1c),]


ans1c_phase3 <- ts_phase3(startvalues = ESTIM, net1 = s501, net2= s502, statistics = STATS, ccovar = DF, itef3 = 10, parallel = TRUE, returnDeps = TRUE)

#which should be equivalent to: 
# ans1c_phase3 <- ts_phase3(ans = ts_ans1c, startvalues = ESTIM, itef3 = 10, parallel = TRUE, returnDeps = TRUE)
...
```{r}
(colMeans(ts_covtheta$zdevs[,1:9]) - ts_targets(net1 = s501, net2 = s502, statistics = STATS, ccovar = DF)) /  (diag(stats::cov(ts_covtheta$zdevs[,1:9])))

sqrt(diag(ts_covtheta$covtheta))

zbar <- colMeans(ts_covtheta$zdevs[,1:9])
zobs <- ts_targets(net1 = s501, net2 = s502, statistics = STATS, ccovar = DF)
Sigma <- stats::cov(ts_covtheta$zdevs[,1:9])

(zbar - zobs) / sqrt(diag(Sigma))

sumsq <- 0
for (i in 1:100) {
  sumsq <- sumsq + ts_covtheta$zdevs[i,1:9] %*% t(ts_covtheta$zdevs[i,1:9])
}

Sigma2 <-  sumsq/1000 - (zbar %*% t(zbar))
tstats <- (zbar - zobs) / sqrt(diag(Sigma))

tconv.max <- sqrt(t(zbar - zobs) %*% solve(Sigma)  %*% (zbar - zobs))


ts_covtheta$tstat
ts_covtheta$tconv.max
...
```
```{r}
ts_covtheta <- ts_phase3(startvalues = ESTIM, net1 = s501, net2= s502, statistics = STATS, ccovar = DF, itef3 = 10, p2step=c(1,0,0), parallel = TRUE)

ts_covtheta$zdevs
(colMeans(ts_covtheta$zdevs[,1:9]) - ts_targets(net1 = s501, net2 = s502, statistics = STATS, ccovar = DF)) / sqrt(diag(cov(ts_covtheta$zdevs[,1:9])))

SE <- sqrt(diag(ts_covtheta$covtheta))
```
