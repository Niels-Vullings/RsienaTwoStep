...
```{r, eval=FALSE}
myalgorithm <- sienaAlgorithmCreate(cond = FALSE, findiff = TRUE) #toggle set conditional to retrieve the rate parameter in theta!
```
...
```{r}
#retrieve the estimates from phase2. Given the oscilations, let us take the mean of the last 250!
...
ans1c_phase3 <- ts_phase3(startvalues = ESTIM, net1 = s501, net2= s502, statistics = STATS, ccovar = DF, parallel = TRUE, returnDeps = TRUE, itef3 = 1000)
...
### ministep
```{r}
ts_ansMS <- ts_estim(net1 = s501, net2= s502, statistics = STATS, ccovar = DF, parallel = TRUE, returnDeps = TRUE)
```
