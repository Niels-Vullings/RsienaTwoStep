...
ESTIM_meaned <- colMeans(ts_ans1c[900: nrow(ts_ans1c),])
fshowdf(data.frame(estim_naive = ESTIM_naive, estim_meaned = ESTIM_meaned))
ans3
```
...
ESTIM <- ans3$theta
ans1c_phase3 <- ts_phase3(startvalues = ESTIM, net1 = s501, net2= s502, statistics = STATS, ccovar = DF, parallel = TRUE, returnDeps = FALSE, itef3 = 20, verbose = TRUE)

#which should be equivalent to: 
# ans1c_phase3 <- ts_phase3(ans = ts_ans1c, startvalues = ESTIM, parallel = TRUE, returnDeps = TRUE)
```
...
We will estimate phase1 ourselves and will estimate phase2 and 3 in one go. 



```{r}
### simultaneity
#ts_ansS <- ts_estim(net1 = s501, net2= s502, statistics = STATS, ccovar = DF, parallel = TRUE, returnDeps = TRUE, p2step = c(0,1,0))
#no convergence. rate becomes very large, degree very negative. 

ts_ansS <- ts_estim(ans3, nite=250, conv = 0.02, parallel = TRUE, returnDeps = TRUE, p2step = c(0,1,0), itef3 = 250)

fsave(ts_ansS, "ts_ansS.rda")

### weak coordination
ts_ansWC <- ts_estim(net1 = s501, net2= s502, statistics = STATS, ccovar = DF, parallel = TRUE, returnDeps = TRUE, p2step = c(0,1,0), dist1 = 2, dist2 = 2, conv = 0.02, nite = 250, itef3 = 250)
fsave(ts_ansWC, "ts_ansWC.rda")

### strict coordination
ts_ansSC <- ts_estim(net1 = s501, net2= s502, statistics = STATS, ccovar = DF, parallel = TRUE, returnDeps = TRUE, p2step = c(0,1,0), dist1 = 2)
fsave(ts_ansSC, "ts_ansSC.rda")

### simstep
ts_ansST <- ts_estim(net1 = s501, net2= s502, statistics = STATS, ccovar = DF, parallel = TRUE, returnDeps = TRUE, p2step = c(0,1,0), dist1 = 2)
