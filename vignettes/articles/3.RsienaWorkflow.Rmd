...
```{r, eval=FALSE}
sims1 <- f_sims(nsims=1000, parallel=TRUE, net=s501, rate=summary(ans1)$theta[1], statistics=list(f_degree, f_recip), parameters=summary(ans1)$theta[2:3], p2step=0, chain=FALSE)
```
...
```{r, eval=FALSE}
sims2 <- f_sims(nsims=1000, parallel=TRUE, net=s501, rate=summary(ans1)$theta[1], statistics=list(f_degree, f_recip), parameters=summary(ans1)$theta[2:3], p2step=1, chain=FALSE)
```
...
### 3.2.3. Weak coordination 

```{r, eval=FALSE}
sims3 <- f_sims(nsims=1000, parallel=TRUE, net=s501, rate=summary(ans1)$theta[1], statistics=list(f_degree, f_recip), parameters=summary(ans1)$theta[2:3], p2step=1, dist1=2, dist2=2, chain=FALSE)
```
...
### 3.2.4. Strict coordination 

```{r, eval=FALSE}
sims4 <- f_sims(nsims=1000, parallel=TRUE, net=s501, rate=summary(ans1)$theta[1], statistics=list(f_degree, f_recip), parameters=summary(ans1)$theta[2:3], p2step=1, dist1=2, chain=FALSE)
```
...
```{r}
df_ts1 <- f_dyads(sims=sims1, simtype="ministep") 
df_ts2 <- f_dyads(sims=sims2, simtype="twostep-simultaneity")
df_ts3 <- f_dyads(sims=sims3, simtype="twostep-weak coordination") 
df_ts4 <- f_dyads(sims=sims4, simtype="twostep-strict coordination")
```
...
```{r}
df <- rbind(dfrs_siena, df_ts1, df_ts2, df_ts3, df_ts4)



p <- ggplot(df, aes(x=x, y=y, fill=factor(type, levels=c("rsiena", "ministep", "twostep-simultaneity", "twostep-weak coordination", "twostep-strict coordination"))) ) + 
...
p <- ggplot(df_sel, aes(x=x, y=y, fill=factor(type, levels=c("rsiena", "ministep", "twostep-simultaneity", "twostep-weak coordination", "twostep-strict coordination"))) ) + 
  geom_violin(position=position_dodge(.8)) + 
  stat_summary(fun = mean,
               geom = "errorbar",
               fun.max = function(x) mean(x) + sd(x),
...
```{r, eval=FALSE}
#ministep
sims1m2 <- f_sims(nsims=1000, parallel=TRUE, net=s501, rate=summary(ans2)$theta[1], statistics=list(f_degree, f_recip, f_cycle3), parameters=summary(ans2)$theta[2:4], p2step=0, chain=FALSE)
fsave(sims1m2, file="sims1m2.Rdata")

#simultaneity
sims2m2 <- f_sims(nsims=1000, parallel=TRUE, net=s501, rate=summary(ans2)$theta[1], statistics=list(f_degree, f_recip, f_cycle3), parameters=summary(ans2)$theta[2:4], p2step=1, chain=FALSE)
fsave(sims2m2, file="sims2m2.Rdata")

#weak coordination
sims3m2 <- f_sims(nsims=1000, parallel=TRUE, net=s501, rate=summary(ans2)$theta[1], statistics=list(f_degree, f_recip, f_cycle3), parameters=summary(ans2)$theta[2:4], p2step=1, dist1=2, dist2=2, chain=FALSE)
fsave(sims3m2, file="sims3m2.Rdata")

#strict coordination
sims4m2 <- f_sims(nsims=1000, parallel=TRUE, net=s501, rate=summary(ans2)$theta[1], statistics=list(f_degree, f_recip, f_cycle3), parameters=summary(ans2)$theta[2:4], p2step=1, dist1=2, chain=FALSE)
...
```{r}
df_ts1m2 <- f_dyads(sims=sims1m2, simtype="ministep") 
df_ts2m2 <- f_dyads(sims=sims2m2, simtype="twostep-simultaneity")
df_ts3m2 <- f_dyads(sims=sims3m2, simtype="twostep-weak coordination") 
df_ts4m2 <- f_dyads(sims=sims4m2, simtype="twostep-strict coordination")
```
...
```{r}
df <- rbind(dfrs_sienam2, df_ts1m2, df_ts2m2, df_ts3m2, df_ts4m2)
...
```{r}
df_ts1m2 <- f_triads(sims=sims1m2, simtype="ministep") 
df_ts2m2 <- f_triads(sims=sims2m2, simtype="twostep-simultaneity")
df_ts3m2 <- f_triads(sims=sims3m2, simtype="twostep-weak coordination") 
df_ts4m2 <- f_triads(sims=sims4m2, simtype="twostep-strict coordination")
```
