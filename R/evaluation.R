...
#' @param ego numeric, the ego for which we are going to calculate how its evaluates the network.
...
#' parameters=c(-2,1))
#' ts_eval(net=net1, ego=10, ccovar=df_ccovar, statistics=list(ts_degree, ts_recip, ts_transTrip, ts_transMedTrip, list(ts_egoX, "cov1")),
#'  parameters=c(-2,2,7,7,1))

ts_eval <- function(net, ego, statistics, ccovar, parameters) {
